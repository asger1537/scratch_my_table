...
describeExperiment('Gemini experiment 10', () => {
  it(
    'runs a leaf-constrained authoring-IR schema without assistant prefill while keeping curated examples',
    async () => {
      const { results } = await executeBenchmarkOpsExperiment({
        experimentTitle: 'Experiment 10',
        schemaPath: path.resolve(process.cwd(), '.tools', 'ai-experiments', 'experiment_10_schema.json'),

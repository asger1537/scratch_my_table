...
describeExperiment('Gemini experiment 1', () => {
  it(
    'runs the current schema against the benchmark prompts and writes a report',
    async () => {
      const { results } = await executeSchemaExperiment({
        experimentName: 'Experiment 1',
        schemaPath: path.resolve(process.cwd(), '.tools', 'ai-experiments', 'experiment_1_schema.json'),
        reportPath: path.resolve(process.cwd(), '.tools', 'ai-experiments', 'experiment_1_report.md'),
      });

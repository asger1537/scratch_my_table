...
export const AI_CANONICAL_RUNTIME_DOCUMENTATION_LINES = [
  ...
  '- scopedRule cases are cumulative, not exclusive: every matching case applies. Do not review scopedRule cases as first-match-wins.',
  '- In scopedRule, later cases see the evolved current cell value after earlier matching cases have applied.',
  '- It is valid for two scopedRule cases to use the same condition when the first case changes the cell and the second case re-checks the evolved cell.',
  '- In scopedRule, if no case matches and there is no defaultPatch, the cell is preserved unchanged.',
  ...
  '- Example: one scopedRule may first fill empty Email from Email (2), then use a later isEmpty(value) case to color the cell red if the fallback was also empty.',
  ...
  '- A fallback-only scopedRule such as "if current Email is empty, set it to Email (2)" does not clear non-empty Email cells.',
